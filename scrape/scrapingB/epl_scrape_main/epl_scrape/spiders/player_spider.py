...
class PlayerSpider(Spider):
    name = "player"
    custom_settings = {"FEEDS": {"player.jsonl": {"format": "jsonl"}}}
    ...
    def start_requests(self):
        for start_url, season_value, season_key in self.gen_start_urls():
            yield Request(
                url=start_url,
                callback=self.parse,
                meta={"season_value": season_value, "season": season_key},
            )
    ...
    #             f"https://footballapi.pulselive.com/football/stats/player/{player_id}?comps=1&compSeasons={season_value}",
    #             meta={"season": season},
    #             callback=self.parse_player,
    #         )

    def parse(self, response):
        import json  # Ensure you import JSON if not already done
        ...
        players = season_data["content"]
        print(
            f"Number of players: {len(players)}"
        )  # Print the number of players

        # Sort players by their ID (smallest to biggest)
        sorted_players = sorted(players, key=lambda player: int(player["id"]))

        for _player in sorted_players:
            player_id = str(int(_player["id"]))
            yield response.follow(
                f"https://footballapi.pulselive.com/football/stats/player/{player_id}?comps=1&compSeasons={season_value}",
                meta={"season": season},
